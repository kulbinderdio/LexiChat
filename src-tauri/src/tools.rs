...
pub fn all_builtin_schemas() -> Vec<serde_json::Value> {
    vec![
        ...
        schema("compose_email", "Build a base64url-encoded RFC 2822 email ready for the Gmail API. Returns ONLY the raw base64url string — use the entire return value as the 'raw' field in gmail_sendmessage, with no modification.",
            ...
            vec!["to","subject","body"]),
        schema("generate_image", "Generate an image from a text description, OR edit an image the user attached, using the local offline image model. Use this whenever the user asks you to create, draw, generate, illustrate, paint, or make an image; and ALSO when they ask you to edit, restyle, or repaint an attached photo (e.g. 'make the building in this photo pink') — for that, pass source_image. The result is displayed inline automatically — refer to it as \"shown above\". To put a result into a slide deck or document: embed it in a create_artifact HTML slide via <img src=\"{{figure:N}}\"> (N = generation order), OR read it in run_python from the path the tool result reports (/work/data/generated_image_N.png) for python-pptx add_picture. Do NOT re-generate images to reuse them. NOTE: this edits the whole image toward the prompt (image-to-image); it is the right tool to recolour/restyle a scene or object while keeping the composition, but it is NOT pixel-exact — for precise, deterministic edits (exact colour swaps, crop, overlay text) use run_python with Pillow instead.",
            vec![("prompt","string","A detailed description of the image to create — or, when editing, of the desired result (describe the whole scene as it should look after the edit, e.g. 'a street with the foreground building painted pink'). Be specific about subject, style, colours, and composition."),
                 ("negative_prompt","string","Things to avoid in the image (optional)."),
                 ("source_image","string","To EDIT an attached image instead of creating a new one: the /work/uploads/<filename> path of an image the user attached ANYWHERE in this conversation — the current message or an earlier one. Earlier attachments stay editable; do not claim a photo is no longer attached. Omit to generate from scratch. Only attached images can be used."),
                 ("strength","number","Edit strength for source_image, 0.0–1.0 (optional, default 0.6, or 0.85 when mask_regions is set). Lower = stay closer to the original photo; higher diverges more. Ignored without source_image."),
                 ("mask_regions","string","To change ONLY part of source_image and keep the rest of the photo pixel-identical (e.g. 'the building', 'the sky'): the region(s) to edit, as normalized (0..1) shapes separated by ';'. Each is 'rect x y w h' (top-left x,y + width,height) or 'ellipse cx cy rx ry' (centre + radii). Estimate the region from the image you can see. Example for a foreground building lower-left: 'rect 0 0.35 0.45 0.65'. If the user painted a region on the image, it's used automatically and you can omit this. Omit to edit the whole image."),
                 ("size","integer","New images: square size in px, e.g. 512, 768, 1024. When editing (source_image): caps the longer edge; the original aspect ratio is kept (optional)."),
                 ("steps","integer","Sampling steps — more is slower but can be higher quality; Turbo models want ~4 (optional)."),
